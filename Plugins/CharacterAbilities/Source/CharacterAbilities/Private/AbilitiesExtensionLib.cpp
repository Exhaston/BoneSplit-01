...
UAbilitySystemComponent* UAbilitiesExtensionLib::GetAbilitySystemSafe(AActor* TargetActor,
	ECAAbilitySystemGetResult& OutResult)
{
	if (!IsValid(TargetActor))
	{
		OutResult = ECAAbilitySystemGetResult::Invalid;
		return nullptr;
	}
	...
	OutResult = ECAAbilitySystemGetResult::Valid;
	return Asc;
}
	...
	UAbilitySystemComponent* FirstAsc, UAbilitySystemComponent* SecondAsc, const FGameplayTag ParentTag)
{
	if (!FirstAsc || !SecondAsc)
	{
		...
		UE_LOG(CharacterAbilities, Error, TEXT("%s"), *DebugNullAsc)
		return false;
	}
...
bool UAbilitiesExtensionLib::IsActorMoving(AActor* InActor)
{
	if (!InActor)
	{
		UE_LOG(CharacterAbilities, Error, TEXT("IsActorMoving executed with null actor!"))
		return false;
	}
...
bool UAbilitiesExtensionLib::IsCharacterGrounded(ACharacter* InCharacter)
{
	if (!InCharacter)                                                                
	{                                                                               
		UE_LOG(CharacterAbilities, Error, TEXT("IsCharacterGrounded executed with null Character!"))
		return false;
	}
...
EMovementMode UAbilitiesExtensionLib::GetCharacterMovementMode(ACharacter* InCharacter)
{
	if (!InCharacter)                                                                
	{                                                                               
		UE_LOG(CharacterAbilities, Error, TEXT("GetCharacterMovementMode executed with null Character!"))
		return MOVE_None;
	}
...
int32 UAbilitiesExtensionLib::GetCharacterCustomMovementMode(ACharacter* InCharacter)
{
	if (!InCharacter)                                                                
	{                                                                               
		UE_LOG(CharacterAbilities, Error, TEXT("GetCharacterCustomMovementMode executed with null Character!"))
		return -1;
	}
