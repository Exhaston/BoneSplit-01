...
AClientAuthoritativeCharacter::AClientAuthoritativeCharacter(const FObjectInitializer& ObjectInitializer) :
	Super(ObjectInitializer.SetDefaultSubobjectClass<UClientAuthoritativeCharacterMovementComponent>(ACharacter::CharacterMovementComponentName))
{
}
...
void AClientAuthoritativeCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AClientAuthoritativeCharacter, bHasOverrideRootMotion, COND_SimulatedOnly);
}

void AClientAuthoritativeCharacter::PossessedBy(AController* NewController)
{
...
{
	Cast<UClientAuthoritativeCharacterMovementComponent>(GetCharacterMovement())->ClientAuthoritativeMove_ServerReceive(MoveData);
}

void AClientAuthoritativeCharacter::PostNetReceiveBasedMovement(const FClientAuthoritativeMoveData& MoveData)
{
...
void AClientAuthoritativeCharacter::PostNetReceiveVelocity(const FVector& NewVelocity)
{
...
void AClientAuthoritativeCharacter::PostNetReceiveLocationAndRotation()
{
	if (GetRemoteRole() == ROLE_AutonomousProxy)
	{
		auto Movement = Cast<UClientAuthoritativeCharacterMovementComponent>(GetCharacterMovement());
		...
		{
			const FClientAuthoritativeMoveData& MovementData = Movement->GetServerLatestMoveData();
			const FVector OldLocation = GetActorLocation();
...
void AClientAuthoritativeCharacter::OnRep_ReplicatedMovement()
{
