...
DEFINE_LOG_CATEGORY_STATIC(LogClientAuthoritativeCharacterMovement, Log, All);
...
void UClientAuthoritativeCharacterMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	SCOPED_NAMED_EVENT(UClientAuthoritativeCharacterMovementComponent_TickComponent, FColor::Yellow);
...
void UClientAuthoritativeCharacterMovementComponent::SimulatedTick(float DeltaSeconds)
{
	SCOPE_CYCLE_COUNTER(STAT_CharacterMovementSimulated);
	checkSlow(CharacterOwner != nullptr);

	if (CharacterOwner->IsReplicatingMovement() && UpdatedComponent && !Cast<AClientAuthoritativeCharacter>(CharacterOwner)->bHasOverrideRootMotion)
	{
...
void UClientAuthoritativeCharacterMovementComponent::ControlledCharacterMove(const FVector& InputVector, float DeltaSeconds)
{
	{
	...
	Cast<AClientAuthoritativeCharacter>(CharacterOwner)->bHasOverrideRootMotion = CharacterOwner->GetCharacterMovement()->CurrentRootMotion.HasOverrideVelocity();
...
void UClientAuthoritativeCharacterMovementComponent::SimulateMovement(float DeltaTime)
{
	...
	// Scoped updates can improve performance of multiple MoveComponent calls.
	{
		...
		if (bNetworkUpdateReceived)
		{
			...
			UE_LOG(LogClientAuthoritativeCharacterMovement, Verbose, TEXT("Proxy %s received net update"), *CharacterOwner->GetName());
		...
		{
			UE_LOG(LogClientAuthoritativeCharacterMovement, Verbose, TEXT("Proxy %s simulating movement"), *GetNameSafe(CharacterOwner));
			...
			UE_LOG(LogClientAuthoritativeCharacterMovement, Verbose, TEXT("Proxy %s SKIPPING simulate movement"), *GetNameSafe(CharacterOwner));
...
void UClientAuthoritativeCharacterMovementComponent::SmoothClientPosition(float DeltaSeconds)
{
...
void UClientAuthoritativeCharacterMovementComponent::SmoothCorrection(const FVector& OldLocation, const FQuat& OldRotation, const FVector& NewLocation, const FQuat& NewRotation)
{
	...
	else if (FNetworkPredictionData_Client_Character* ClientData = GetPredictionData_Client_Character())
	{
		...
		UE_LOG(LogClientAuthoritativeCharacterMovement, Verbose, TEXT("Proxy %s SmoothCorrection(%.2f)"), *GetNameSafe(CharacterOwner), FMath::Sqrt(DistSq));
		...
		if (ClientData->SmoothingClientTimeStamp > ClientData->SmoothingServerTimeStamp)
		{
			...
			UE_LOG(LogClientAuthoritativeCharacterMovement, VeryVerbose, TEXT("SmoothCorrection: Pull back client from ClientTimeStamp: %.6f to %.6f, ServerTimeStamp: %.6f for %s"),
		...
		UE_LOG(LogClientAuthoritativeCharacterMovement, VeryVerbose, TEXT("SmoothCorrection: WorldTime: %.6f, ServerTimeStamp: %.6f, ClientTimeStamp: %.6f, Delta: %.6f for %s"),
...
void UClientAuthoritativeCharacterMovementComponent::ClientAuthoritativeMove_ServerReceive(const FClientAuthoritativeMoveData& MoveData)
{
	...
	auto Character = Cast<AClientAuthoritativeCharacter>(CharacterOwner);
...
void UClientAuthoritativeCharacterMovementComponent::ReplicateAutonomousMoveToServer()
{
	...
	const float NetMoveDelta = FMath::Clamp(GetClientAuthoritativeNetSendDeltaTime(PC), 1.f / 120.f, 1.f / 5.f);
	...
	// Send move to server
	SendClientAuthoritativeMove();
}

float UClientAuthoritativeCharacterMovementComponent::GetClientAuthoritativeNetSendDeltaTime(const APlayerController* Controller) const
{
...
void UClientAuthoritativeCharacterMovementComponent::SendClientAuthoritativeMove()
{
	// Pass through RPC call to character, there is less RPC bandwidth overhead when used on an Actor rather than a Component.
	Cast<AClientAuthoritativeCharacter>(CharacterOwner)->ClientAuthoritativeMove(GenerateClientAuthoritativeMoveData());

	MarkForClientCameraUpdate();
}

FClientAuthoritativeMoveData UClientAuthoritativeCharacterMovementComponent::GenerateClientAuthoritativeMoveData()
{
	FClientAuthoritativeMoveData Data;
	...
	Data.bHasOverrideRootMotion = Cast<AClientAuthoritativeCharacter>(CharacterOwner)->bHasOverrideRootMotion;
