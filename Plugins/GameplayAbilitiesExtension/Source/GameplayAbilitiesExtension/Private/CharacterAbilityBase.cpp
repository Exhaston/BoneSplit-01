	...
	FGameplayTagContainer* OptionalRelevantTags) const
{
	if (!CharacterAvatar.IsValid())
	{
		...
		UE_LOG(AbilitiesExtension, Warning, TEXT("%s"), *DebugNoCharacter);
		return false;
	}
	...
	const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo) const
{
	...
	if (CooldownGE)
	{
		...
		SpecHandle.Data.Get()->SetSetByCallerMagnitude(DefaultSetByCallerTags::SetByCaller_Duration, CooldownDuration.GetValueAtLevel(GetAbilityLevel()) * (1 - GetCooldownModifier()));
