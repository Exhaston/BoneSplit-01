...
FObjectThumbnail* FThumbnailExporterRenderer::GenerateThumbnail(FThumbnailCreationConfig& CreationConfig, UObject* InObject, const FPreCreateThumbnail& CreationDelegate)
{
	...
	if (const FThumbnailRenderingInfo* RenderInfo = GUnrealEd ? GUnrealEd->GetThumbnailManager()->GetRenderingInfo(GetMutableDefault<UThumbnailExporterThumbnailDummy>()) : nullptr; 
		RenderInfo && RenderInfo->Renderer)
	{
		...
		constexpr ThumbnailTools::EThumbnailTextureFlushMode::Type TextureFlushMode = ThumbnailTools::EThumbnailTextureFlushMode::AlwaysFlush;
...
static FThumbnailRenderTargetResource CreateThumbnailRenderTarget(const uint32 InImageWidth, const uint32 InImageHeight, const FLinearColor ClearColor)
{
	...
	const uint32 InImageWidth, const uint32 InImageHeight, ThumbnailTools::EThumbnailTextureFlushMode::Type InFlushMode, FObjectThumbnail* OutThumbnail, const FPreCreateThumbnail& CreationDelegate)
{
	...
	FThumbnailRenderingInfo* RenderInfo = GUnrealEd ? GUnrealEd->GetThumbnailManager()->GetRenderingInfo(GetMutableDefault<UThumbnailExporterThumbnailDummy>()) : nullptr;
	...
	if (RenderInfo && RenderInfo->Renderer)
	{
		...
		TGuardValue Unattended(GIsRunningUnattendedScript, true);
	...
	if (OutThumbnail)
	{
		...
		LDRThumbnail.RenderTargetResource->ReadPixelsPtr(reinterpret_cast<FColor*>(OutData.GetData()), FReadSurfaceDataFlags(), InSrcRect);

		TArray<uint8> AlphaData;
		AlphaData.AddUninitialized(OutThumbnail->GetImageWidth() * OutThumbnail->GetImageHeight() * sizeof(FColor));

		AlphaThumbnail.RenderTargetResource->ReadPixelsPtr(reinterpret_cast<FColor*>(AlphaData.GetData()), FReadSurfaceDataFlags(), InSrcRect);

		FColor* Color = reinterpret_cast<FColor*>(OutData.GetData());
		FColor* Alpha = reinterpret_cast<FColor*>(AlphaData.GetData());
		if (CreationConfig.InvertBackgroundAlpha())
		{
			for (; Color < reinterpret_cast<FColor*>(OutData.GetData() + OutData.Num()); ++Color, ++Alpha)
			{
				Color->A = 255 - Alpha->A;
			}
		}
