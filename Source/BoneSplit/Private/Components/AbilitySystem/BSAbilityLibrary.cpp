...
bool UBSAbilityLibrary::HasNoMatchingFaction(UAbilitySystemComponent* Asc, UAbilitySystemComponent* OtherAsc)
{
	return !HasMatchingFaction(Asc, OtherAsc);
}

bool UBSAbilityLibrary::IsCharacterGrounded(ACharacter* Character)
{
	return !Character->GetCharacterMovement()->IsFalling();
}

bool UBSAbilityLibrary::CanSeeTargetActor(AActor* OriginActor, AActor* TargetActor)
{
	return CanSeeTargetLocation(
		OriginActor->GetWorld(), 
		OriginActor->GetActorLocation(), 
		TargetActor->GetActorLocation());
}

bool UBSAbilityLibrary::CanSeeTargetLocation(UWorld* World, const FVector OriginLocation, const FVector TargetLocation)
{
	check(World);
...
{
	check(InOriginActor);
	return GetActorsInRadiusFromPoint(InOriginActor->GetWorld(), InOriginActor->GetActorLocation(), Radius);
}

TArray<AActor*> UBSAbilityLibrary::GetActorsInRadiusFromPoint(
	UWorld* World, const FVector OriginLocation, const float Radius)
{
	check(World);
...
void UBSAbilityLibrary::TryActivateAbilityWeighted(UAbilitySystemComponent* OwnerAsc,
...
void UBSAbilityLibrary::LazyApplyEffectToTarget(UGameplayAbility* Ability,
...
void UBSAbilityLibrary::ApplyPayloadEffect(UGameplayAbility* Ability, const FGameplayTagContainer EventTags,
	const FGameplayEventData Payload, TArray<TSubclassOf<UBSGameplayEffect>> Effects, FBSTargetFilter Filter,
	const float EffectLevel)
{
	check(Ability);
	if (Effects.IsEmpty()) return;
	if (Payload.Target && EventTags.HasTagExact(Payload.EventTag))
	{
		...
		{
			ContextHandle.AddOrigin(Payload.ContextHandle.GetOrigin());
		}
...
bool UBSAbilityLibrary::CanMobReachLocation(AActor* InMob, const FVector TargetLocation)
{
...
bool UBSAbilityLibrary::CheckMobGrounded(AActor* InMob)
{
...
TArray<AActor*> UBSAbilityLibrary::FilterArrayByReachable(AActor* InOriginActor, TArray<AActor*> InActors)
{
	TArray<AActor*> OutActors;
...
TArray<AActor*> UBSAbilityLibrary::FilterArrayByVisible(
...
bool UBSAbilityLibrary::CheckTargetVisibility(UObject* WorldContext, const FVector InOrigin, const AActor* InTargetActor)
{
	const UWorld* World = GEngine->GetWorldFromContextObjectChecked(WorldContext);
	FHitResult HitResult; 
...
TArray<AActor*> UBSAbilityLibrary::GetActorsInRadiusDep(
...
AActor* UBSAbilityLibrary::PickRandomTargetFromArray(TArray<AActor*> InActors)
{
	if (InActors.IsEmpty())
	{
		return nullptr;
...
AActor* UBSAbilityLibrary::PickClosestTargetFromArray(AActor* InOriginActor, TArray<AActor*> InActors)
{
...
AActor* UBSAbilityLibrary::PickFarthestTargetFromArray(AActor* InOriginActor, TArray<AActor*> InActors)
{
	if (!InOriginActor || InActors.IsEmpty())
	{
		return nullptr;
...
TArray<AActor*> UBSAbilityLibrary::GetThreatActors(AActor* InTarget)
{
...
AActor* UBSAbilityLibrary::GetHighestThreatActor(AActor* InTarget)
{
	if (UBSThreatComponent* ThreatComponent = GetThreatComponent(InTarget))
	{
...
AActor* UBSAbilityLibrary::GetLowestThreatActor(AActor* InTarget)
{
	if (UBSThreatComponent* ThreatComponent = GetThreatComponent(InTarget))
	{
...
UBSThreatComponent* UBSAbilityLibrary::GetThreatComponent(AActor* InTarget)
{
...
TArray<AActor*> UBSAbilityLibrary::SliceShapeOverlap(
...
void UBSAbilityLibrary::DrawDebugSlice(
...
FVector UBSAbilityLibrary::GetSwizzledDirectionFromRotation(const TEnumAsByte<EAxis::Type> InAxis,
...
UBSAbilitySystemComponent* UBSAbilityLibrary::GetBSAbilitySystemComponent(AActor* ActorToTest)
{
	if (ActorToTest)
	{
...
TSubclassOf<UGameplayAbility> UBSAbilityLibrary::BP_GetAbilityByWeight(UAbilitySystemComponent* Asc,
	const TMap<TSubclassOf<UGameplayAbility>, float> Map)
{
	return GetAbilityByWeight(Asc, Map);
}

TSubclassOf<UGameplayAbility> UBSAbilityLibrary::GetAbilityByWeight(UAbilitySystemComponent* Asc,
                                                                            const TMap<TSubclassOf<UGameplayAbility>, float>& Map)
{
...
bool UBSAbilityLibrary::CanActivateAbility(const UAbilitySystemComponent* Asc,
