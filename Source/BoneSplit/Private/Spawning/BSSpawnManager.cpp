...
#include "Spawning/BSSpawnManager.h"
...
#include "Spawning/BSSpawnBatch.h"
#include "Spawning/BSSpawnData.h"
...
void ABSSpawnManager::SpawnNextMob()
{
    ...
    {
        if (ABSMobCharacter* Mob = Spawner->SpawnMob(Info.SpawnedMob))
        {
            LiveMobs.Add(Mob);
            ++TotalMobsSpawned;
            ...
            Mob->OnMobDiedDelegate.AddDynamic(this, &ABSSpawnManager::OnMobDied);
        }
    }
...
void ABSSpawnManager::OnMobDied(ABSMobCharacter* Mob)
{
    LiveMobs.Remove(Mob);
...
{
    for (const TObjectPtr<ABSMobCharacter>& Mob : LiveMobs)
    {
